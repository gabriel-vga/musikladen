              ...
              <div class="container">
                <h3>ÁREA DO ADMNISTRADOR</h3>
                  ...
                  if (isset($_POST['excluir'])) {//Botão excluir pressionado
                    if (!empty($_POST['cat'])) {//Verifica se existem categorias
                      foreach ($_POST['cat'] as $cat => $id) {
                        echo $id . "<br>";
                        $sql = "DELETE FROM tbFornecedor WHERE idFornecedor = '$id'";
                        if (mysqli_query($conexao,$sql) ) {
                          echo "<script>alert('Fornecedor(es) apagado(s) com sucesso!')</script>";
                          echo "<script>window.location.href = 'cadastrofornecedor.php'</script>";//Limpando URL
                        } else {
                  ...
                  if (isset($_GET['acao'])){
                    ...
                    $idfornecedor = $_GET['cat'];//Guarda o id do fornecedor
                    
                    if (isset($_POST['editar'])) {
                      $fornecedor = $_POST['fornecedor'];//Recebe o novo nome que vem do form
                      $sql = "UPDATE tbfornecedor SET NomeFornecedor = '$fornecedor' WHERE idFornecedor = '$idfornecedor'";
                    if (mysqli_query($conexao,$sql) ) {
                    ...
                    <fieldset><legend>.::Editar Fornecedor::.</legend>
                    <form action="" method="post">
                      <p>* obrigatório </p>
                      <p>
                      <label for="fornecedor">Fornecedor: </label>
                      <input type="text" name="fornecedor"> * <?php echo $fornecedorErro ?>
                      </p>
                  ...
                  if (!isset($_GET['acao'])) {//pressionou o botão
                    ...
                    if (empty($_POST['fornecedor'])) { //não preencheu o nome do fornecedor
                      $fornecedorErro = "obrigatório";
                    } else { // preencheu corretamente
                      $fornecedor = $_POST['fornecedor'];
                      //inciar o cadastro
                        
                      $sql = "INSERT INTO tbfornecedor (NomeFornecedor) VALUES ('$fornecedor')";
                      //inserção do dado
                      if (mysqli_query($conexao, $sql)) {
                  ...
                  //Listar os fornecedores cadastradas
                  require 'conexao.php';
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    <form action="" method="post">
                      ...
                      <?php
                    while ($fornecedor = mysqli_fetch_assoc($result)) {
                      ?>
                          <tr>
                        <td><?php echo $fornecedor['idFornecedor'] ?></td>
                        <td><?php echo $fornecedor['NomeFornecedor'] ?></td>
                        <td><a href="?acao=edit&cat=<?php echo $fornecedor['idFornecedor'] ?>">editar</a></td>
                        <td><input type="checkbox" name="cat[]" value="<?php echo $fornecedor['idFornecedor'] ?>"></td>
                        </tr>
                      <?php
                    }
                      ?>
