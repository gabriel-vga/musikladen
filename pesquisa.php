                ...
                require ("conexao.php"); 
                        ...
                        $sql = "SELECT * FROM tbmusica JOIN tbfornecedor ON idmForn = idFornecedor JOIN tbgenero ON idmGenero = idGenero JOIN tbalbum ON idmAlbum = idAlbum JOIN tbcantor ON idmCantor = idCantor JOIN tbcompositor ON idmCompositor = idCompositor WHERE CONCAT_WS( ' ',NomeAlbum,NomeCantor,NomeMusica,NomeCompositor,NomeGenero,NomeFornecedor,Gravadora) LIKE '%$pesquisa%' ORDER BY NomeMusica";
                          
                        $result = mysqli_query($conexao, $sql) or die(mysqli_error($conexao));//guarda o resultado da consulta

                  if (mysqli_num_rows($result) > 0) {
                    ?>
                    <h2 class="font-thin m-b">Resultados da pesquisa:<?php echo " "."$pesquisa" ?><span class="musicbar animate inline m-l-sm" style="width:20px;height:20px">
