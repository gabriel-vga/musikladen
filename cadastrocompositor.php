              ...
              <div class="container">
                <h3>ÁREA DO ADMNISTRADOR</h3>
                  ...
                  if (isset($_POST['excluir'])) {//Botão excluir pressionado
                    if (!empty($_POST['cat'])) {//Verifica se existem categorias
                      foreach ($_POST['cat'] as $cat => $id) {
                        echo $id . "<br>";
                        $sql = "DELETE FROM tbcompositor WHERE idCompositor = '$id'";
                  ...
                  if (isset($_GET['acao'])){
                    ...
                    $idcompositor = $_GET['cat'];//Guarda o id do compositor
                    
                    if (isset($_POST['editar'])) {
                      $compositor = $_POST['compositor'];//Recebe o novo nome que vem do form
                      $sql = "UPDATE tbcompositor SET NomeCompositor = '$compositor' WHERE idCompositor = '$idcompositor'";
                    if (mysqli_query($conexao,$sql) ) {
                    ...
                    <fieldset><legend>.::Editar Compositores::.</legend>
                    <form action="" method="post">
                      <p>* obrigatório </p>
                      <p>
                      <label for="compositor">Compositor: </label>
                      <input type="text" name="compositor"> * <?php echo $compositorErro ?>
                      </p>
                  ...
                  if (!isset($_GET['acao'])) {//pressionou o botão
                    ...
                    if (empty($_POST['compositor'])) { //não preencheu o nome do compositor
                      $compositorErro = "obrigatório";
                    } else { // preencheu corretamente
                      $compositor = $_POST['compositor'];
                      //inciar o cadastro
                        
                      $sql = "INSERT INTO tbcompositor (NomeCompositor) VALUES ('$compositor')";
                      //inserção do dado
                      if (mysqli_query($conexao, $sql)) {
                  ...
                  <fieldset><legend>.::Cadastro de Compositores::.</legend>
                  <form action="" method="post">
                    <p>* obrigatório </p>
                    <p>
                    <label for="compositor">Compositor: </label>
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($compositor = mysqli_fetch_assoc($result)) {
                      ?>
                          <tr>
                        <td><?php echo $compositor['idCompositor'] ?></td>
                        <td><?php echo $compositor['NomeCompositor'] ?></td>
                        <td><a href="?acao=edit&cat=<?php echo $compositor['idCompositor'] ?>">editar</a></td>
                        <td><input type="checkbox" name="cat[]" value="<?php echo $compositor['idCompositor'] ?>"></td>
                        </tr>
                      <?php
                    }
                      ?>
